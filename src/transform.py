""" Módulo responsável pela transformação e limpeza dos dados brutos """
...
def drop_columns(df: pd.DataFrame, sheet: int = 1) -> pd.DataFrame:
    """Remove colunas desnecessárias de acordo com a planilha de origem."""
    if sheet == 1:
...
def join_dataframes(df_1: pd.DataFrame, df_2: pd.DataFrame) -> pd.DataFrame:
    """Realiza a junção (outer join) de dois DataFrames."""
    return df_1.join(df_2, how='outer')


def rename_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Renomeia as colunas do DataFrame para um padrão interno."""
...
def concatenate_dfs(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
    """Concatena dois DataFrames verticalmente."""
    return pd.concat([df1, df2], ignore_index=True)


def capitalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Capitaliza a primeira letra das strings nas colunas de características físicas."""
...
def standardize_skin_color(df: pd.DataFrame) -> pd.DataFrame:
    """Padroniza os valores da coluna de cor da pele."""
...
def standardize_hair_color(df: pd.DataFrame) -> pd.DataFrame:
    """Padroniza os valores da coluna de cor do cabelo."""
...
def standardize_eye_color(df: pd.DataFrame) -> pd.DataFrame:
    """Padroniza os valores da coluna de cor dos olhos."""
    df['cor_olhos'] = (
        df['cor_olhos']
...
def address_null_values(df: pd.DataFrame) -> pd.DataFrame:
    """Substitui strings vazias por valores nulos nas colunas de SNPs."""
...
def transform(dfs: list[pd.DataFrame]) -> pd.DataFrame:
    """Executa o pipeline completo de transformação nos DataFrames fornecidos."""
